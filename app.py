...
# --- 介面設定 ---
st.set_page_config(page_title="專業台股分析系統 (整合穩定版)", layout="wide")
    ...
    with st.spinner('正在分析指標...'):
        ...
        if not df.empty:
            ...
            fig.add_trace(go.Candlestick(x=df.index, open=df['open'], high=df['max'], low=df['min'], close=df['close'], name='K線',
                                        ...
                                        increasing_fillcolor='#FF0000', decreasing_fillcolor='#00FF00'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df.index, y=df['MA5'], name='5MA', line=dict(color='gold', width=1.2)), row=1, col=1)
            fig.add_trace(go.Scatter(x=df.index, y=df['MA10'], name='10MA', line=dict(color='cyan', width=1.2)), row=1, col=1)
            ...
            st.error(f"找不到代碼 {stock_id} 的數據，請檢查代碼或 Token。")
